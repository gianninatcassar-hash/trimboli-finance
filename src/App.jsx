    ...
    <header className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-white/70 bg-white/80 border-b border-slate-200">
      <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6" aria-label="Main">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="text-sm text-slate-700 hover:text-sky-700">{l.label}</a>
          ))}
          <a href="#contact" className="inline-flex items-center rounded-xl px-4 py-2 text-sm font-semibold bg-sky-600 text-white shadow hover:shadow-md">
            Free consult
          </a>
        </nav>
        <button
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-slate-200 px-4 py-3 space-y-2">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="block text-sm text-slate-700 hover:text-sky-700">{l.label}</a>
          ))}
          <a href="#contact" className="block rounded-xl px-4 py-2 text-sm font-semibold bg-sky-600 text-white shadow hover:shadow-md text-center">
            Free consult
          </a>
        </div>
      )}
    ...
    <div>
      ...
      <main>
        <section id="home" className="py-20 bg-sky-50">
          <div className="mx-auto max-w-3xl text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-slate-900">Welcome to {BRAND.name}</h1>
            <p className="mt-4 text-lg text-slate-600">
              At {BRAND.name}, led by {BRAND.principal}, we make home loans and refinancing simple, personal, and stress-free.
            </p>
            <div className="mt-6">
              <a href="#contact" className="inline-flex items-center rounded-xl px-6 py-3 text-lg font-semibold bg-sky-600 text-white shadow hover:shadow-md">
                Talk to Rocky today
              </a>
            </div>
          </div>
